...
def cnn_predictor(clf_model,
    ...
    if (counter == config['clf_model']['init_n'] - 1) or ((counter >= config['clf_model']['init_n']) and (len(data) == config['clf_model']['n_preds'])):
        data_df = pd.DataFrame.from_dict(data,
                                         orient='index')
        test_array = np.expand_dims(np.array([np.stack(data_df.frame_array.apply(lambda x: np.array(x)).to_list())]), axis = -1)
        test_features = np.array([data_df[config['clf_model']['model_columns']].values])
        mean_pred = np.mean(clf_model.predict([test_array, test_features])[:, :, 1])
